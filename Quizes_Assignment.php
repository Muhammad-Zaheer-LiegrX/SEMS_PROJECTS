...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES["file"]) && $_FILES["file"]["error"] == 0) {
        $target_dir = "Assig_quiz/"; // Change this to the desired directory for uploaded files
        ...
        if (!in_array($file_type, $allowed_types)) {
            ...
            if (move_uploaded_file($_FILES["file"]["tmp_name"], $target_file)) {
                ...
                $fileType = $_POST['fileType']; // Retrieve the selected file type

                // Database connection
                // Insert the file information into the database with file type
                $sql = "INSERT INTO Assign (filename, filesize, filetype) 
                        VALUES ('$filename', $filesize, '$fileType')";

                if ($conn->query($sql) === TRUE) {
                    $message = "The file " . basename($_FILES["file"]["name"]) . " has been uploaded and is ready to view for the students.";
                    $_SESSION['loginMessage'] = $message;

                    header("location: Quizes_Assignment.php");
...
<header>
  ...
  <div id="main">
	...
	<div class="container mt-5">
		<h2>Upload a Quiz/Assignment/EXam file</h2>


		<form action="Quizes_Assignment.php" method="POST" enctype="multipart/form-data">
...
if ($result->num_rows > 0) {
  while ($row = $result->fetch_assoc()) {
      ...
      <tr>
          ...
          <td>
              <form action="Quizes_Assignment.php" method="POST" style="display:inline;">
